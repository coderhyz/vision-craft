...
export interface ChartBaseProps {
    title: string; // 图表标题 其他都是可选
    // 视觉与交互
    showGrid?: boolean; // 是否显示网格
    showLegend: boolean; // 是否显示图例
    tooltipEnabled: boolean; // 是否启用提示框
}
/**
 * 通用图表数据源配置
 */
...
/**
 * 基础图表组件接口 继承自 BaseComponentNode
 * - 具体图表通过泛型约束自己的 props 结构
 * - dataSource 在组件顶层，不在 props 中
 */
export interface BaseChartComponent<
    // 泛型
    // TProps 必须满足 ChartBaseProps，如果没有传入，就默认使用 ChartBaseProps
...
> extends BaseComponentNode {
    ...
    | "GaugeChart" //仪表图
    | "TreemapChart"; //树状图
    props: TProps; // 具体图表的 props 结构
    dataSource?: ChartDataSource; //通用图表数据源
}
/**
 ...
 */
export interface RootContainerRendererProps extends RendererProps {
